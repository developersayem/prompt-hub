...
export default function CreatePromptModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!navigator.onLine) {
      setShowOfflineDraftModal(true); // show the modal
      return;
    }
  ...
  return (
    <>
      <Dialog
        open={showOfflineDraftModal}
        onOpenChange={setShowOfflineDraftModal}
      >
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>You are offline</DialogTitle>
            <DialogDescription>
              It looks like you’re not connected to the internet. Would you like
              to save your prompt as a draft so you can publish it later?
            </DialogDescription>
          </DialogHeader>
          <div className="flex justify-end gap-3 mt-4">
            <Button
              variant="outline"
              onClick={() => setShowOfflineDraftModal(false)}
            >
              Cancel
            </Button>
            <Button
              onClick={async () => {
                ...
                setShowOfflineDraftModal(false);
              }}
            >
              Save as Draft
            </Button>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full">
                    ...
                    <div className="space-y-2">
                      ...
                      <Combobox<IAiModel>
                        ...
                        value={
                          aiModels.find((m) => m.name === formData.aiModel)
                            ?._id || ""
                        } // <- controlled by name
